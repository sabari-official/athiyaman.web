...
export function ApplyLeader() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Endpoint: POST /api/v1/applications/leader
      await api.post("/applications/leader", {
        full_name: fullName,
        phone,
        email,
        aadhaar,
        ...
        city,
        reason: null, // "Reason" field has been fully removed from the registration requirements
      });

      toast.success("Leader application submitted successfully!");
      setSuccess(true);
    } catch (err) {
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-6 relative">
        ...
        <div className="max-w-md w-full text-center bg-white/70 backdrop-blur-lg border border-white/50 p-8 rounded-3xl shadow-2xl relative animate-fade-in-up">
          ...
          <p className="mt-3 text-sm text-slate-600 leading-relaxed">
            Your leader application has been filed successfully. Our administrators will vet your physical Aadhaar credentials and contact you within 2-3 business days.
          </p>
    ...
    <div className="min-h-screen bg-background py-16 px-4 sm:px-6 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto bg-white/80 backdrop-blur-md border border-white/50 rounded-3xl shadow-2xl overflow-hidden relative animate-fade-in-up">
        ...
        <div className="bg-nav-gradient text-white p-8 relative overflow-hidden">
          ...
          <h1 className="mt-6 text-3xl font-black tracking-tight">Apply as a Team Leader</h1>
          <p className="mt-2 text-sm text-white/85 leading-relaxed max-w-xl">
            Lead local civic action programs, register community members, earn rewards, and help keep India clean. Submit your verified registration details below.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-6 border-t border-slate-100">
            ...
            >
              {loading ? "Filing Application…" : "Submit Application"}
            </button>
          </div>
        </form>
      </div>
